...
export async function POST(req: NextRequest) {
  try {
    ...
    const token = signToken({ agentId: agent.id, name: agent.name, mobile: agent.mobile, isAdmin: agent.isAdmin });
    const response = NextResponse.json({ success: true, name: agent.name, isAdmin: agent.isAdmin });
